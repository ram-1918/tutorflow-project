    ...
    <div class="answer-tab">
        <div class="answer-tab-topnav">
            <div class="name-section">
                <span style="font-size:medium; font-weight:bold"><i class="fa fa-user-circle-o" style="font-size:18px"></i>
                    {{ username }}
                    <!-- <span v-if="!user.data && (answer.user == user.data.user.id)" @click="changeMode()" style="padding-left: 1rem; cursor:pointer">
                        <i class="fa fa-edit" style="font-size:24px"></i>
                    </span> -->
                </span>
                <span style="font-size: smaller; font-weight: lighter;padding-top: 10px;">{{date}} at {{time}}, {{ days }}</span>
            </div>
        ...
        <text-editor :answer="answer.answer" :key="answer.id"></text-editor>

        <!-- <div id="editor-ro"></div> -->
        <!-- <div id="editorAns">{{ answer.answer }}</div> -->
        ...
        <div class="que-tags">
            ...
            <span v-if="answer.topic"><span v-for = "tp in answer.topic.split(',')" :key="tp">
                <base-button v-if="tp" type = "disabled" mode = "tag">{{ tp.toLowerCase() }}</base-button>
            </span></span>
        </div>
...
export default {
    ...
    computed:{
        ...
        days(){
            var days = Math.floor((new Date() - new Date(this.answer.date_answered))/ (24*60*60*1000));
            if (days === 0){ return 'today'}
            else if(days === 1) {return `${days} day ago`}
            else {return `${days} days ago`}
        },
        username(){
            if(this.answer.student_name === 'User') {return 'Anonymous'}
            else{
                var name = this.answer.student_name;
    ...
    methods:{
        changeMode(){this.editMode = !this.editMode},
