<template>
    ...
    <div id="id01" class="modal">
        <form class="modal-content" @submit.prevent="sendToken()" v-if="!token_sent">
        ...
        <form class="modal-content" @submit.prevent="verifyToken()" v-else>
            <span class="error-msg" v-if="this.$store.state.err">{{ this.$store.state.err }}</span><br>
            <span class="login-title">Login</span><br>
            <div class="container">
                <fieldset>
        ...
        <form class="modal-content" @submit.prevent="resetPassword()" v-if="is_verified">
            <span class="error-msg" v-if="this.$store.state.err">{{ this.$store.state.err }}</span><br>
            <span class="login-title">Login</span><br>
            <div class="container">
                <fieldset>
    ...
    methods:{
        sendToken(){
            ...
            if (email_regex.test(this.email)){
                    ...
                    axios.post('http://127.0.0.1:8000/api/forgot-password/', user_data)
                    .then((response) => {
        ...
        verifyToken(){
            ...
            axios.post('http://127.0.0.1:8000/api/verify-fp/', data)
            .then((response) =>{
                console.log(response.data)
