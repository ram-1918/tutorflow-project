    ...
    <div v-if = "quesDetails">
        <div class="answer-container">
            <div class="question-section">
                ...
                <span style="font-size: small; font-weight: 300;">Date Posted: {{ date }} at {{ time }}, {{days}}</span>
            </div> 
            <p>Total Answers: {{ ansDetails.length }}</p>
            <div v-if="ansDetails">
                <!-- {{ ansDetails }} -->
                <div id="answer-section" >
                    <answer-content2 v-for="ans of ansDetails" :key="ans.id"
                    :answer="ans"
...
export default{
    ...
    computed:{
        ...
        month(){return new Date(this.$store.state.selectedQueDetails.date_create).getMonth()},
        time(){return new Date(this.$store.state.selectedQueDetails.date_create).toLocaleTimeString()},
        days(){
            var days = Math.floor((new Date() - new Date(this.$store.state.selectedQueDetails.date_create))/ (24*60*60*1000));
